...
import { wsip, wsport, path, codeList } from '../config'
...
function parsePublishData(topic, jsonContent) {
  if (topic === Cons.TOPIC_LOG || 
    ...
    topic === Cons.TOPIC_GATEWAY || 
		...
		topic === Cons.TOPIC_TRADE_MARKETDATA
	) {
...
////////////////////////////////////////////////////////////
export function tradeLogin(username, password) {
  return new Promise((resolve, reject) => {
    databus.requestOnce(Cons.COMMAND_TRADE_LOGINREQ, "trade", "Trade.LoginReq", "Trade.LoginResp", { 
      fillRequest: function (request) {
				console.log(" - Send LoginInfo Request To Server");
				request.userid = username || ''
				request.passwd = password || ''
				request.instruments = codeList
      },
      handleResponse: function (response) {
				console.log(" - Receive LoginResponse Response From Server:" + JSON.stringify(response));
        return resolve(response);
...
export function subAccount() {
	...
	databus.requestPublishData(Cons.TOPIC_TRADE_MARKETDATA, dispatchPublishMarketData)
	return subscribeList([123, 124], [Cons.TOPIC_TRADE_TRADINGACCOUNT, Cons.TOPIC_TRADE_MARKETDATA])
}
function dispatchPublishTradingAccount(jsonContent) {
	console.log('dispatchPublishTradingAccount')
	console.log(jsonContent)
}

function dispatchPublishMarketData(jsonContent) {
	console.log('dispatchPublishMarketData')
	console.log(jsonContent)
}
